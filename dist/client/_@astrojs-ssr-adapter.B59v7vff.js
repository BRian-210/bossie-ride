...
class RenderContext {
  ...
  static #createNormalizedUrl(requestUrl) {
    ...
    try {
      url2.pathname = validateAndDecodePathname(url2.pathname);
    } catch {
      try {
...
class NodeApp extends App {
  ...
  static createRequest(req, {
    ...
    try {
      ...
      url2 = new URL(`${providedProtocol}://${hostnamePort}`);
    }
    const options = {
...
function createAppHandler(app, options) {
  ...
  const prerenderedErrorPageFetch = originUrl ? (url2) => {

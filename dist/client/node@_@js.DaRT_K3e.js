import { i as isRemoteAllowed, j as joinPaths, a as isRemotePath, r as removeQueryString, b as isParentDirectory } from "./remote.Dx-Q6-YI.js";
import { A as AstroError, E as ExpectedImage, L as LocalImageUsedWrongly, M as MissingImageDimension, U as UnsupportedImageFormat, I as IncompatibleDescriptorOptions, a as UnsupportedImageConversion, t as toStyleString, N as NoImageMetadata, F as FailedToFetchRemoteImageDimensions, b as ExpectedImageOptions, c as ExpectedNotESMImage, d as InvalidImageService, e as createComponent, f as createAstro, g as ImageMissingAlt, m as maybeRenderHead, h as addAttribute, s as spreadAttributes, r as renderTemplate, i as ExperimentalFontsNotEnabled, j as FontFamilyNotFound, u as unescapeHTML } from "./astro/server.C9n97yqK.js";
import "clsx";
...
const baseService = {
  ...
  getURL(options, imageConfig2) {
    ...
    const imageEndpoint = joinPaths("http://localhost:4322", imageConfig2.endpoint.route);
...
const $$Image = createComponent(async ($$result, $$props, $$slots) => {
  const Astro = $$result.createAstro($$Astro$2, $$props, $$slots);
  Astro.self = $$Image;
  const props = Astro.props;
  if (props.alt === void 0 || props.alt === null) throw new AstroError(ImageMissingAlt);
  typeof props.width == "string" && (props.width = parseInt(props.width)), typeof props.height == "string" && (props.height = parseInt(props.height)), (props.layout ?? imageConfig.layout ?? "none") !== "none" && (props.layout ??= imageConfig.layout, props.fit ??= imageConfig.objectFit ?? "cover", props.position ??= imageConfig.objectPosition ?? "center");
  const image = await getImage(props), additionalAttributes = {};
  image.srcSet.values.length > 0 && (additionalAttributes.srcset = image.srcSet.attribute);
  const { class: className, ...attributes } = { ...additionalAttributes, ...image.attributes };
  return renderTemplate`
${maybeRenderHead()}<img data-source-file="/home/rayan/bossie-ride/node_modules/astro/components/Image.astro" data-source-line-start="51" data-source-line-end="51"${addAttribute(image.src, "src")}${spreadAttributes(attributes)}${addAttribute(className, "class")}>`;
}, "/home/rayan/bossie-ride/node_modules/astro/components/Image.astro", void 0);
const $$Astro$1 = createAstro();
const $$Picture = createComponent(async ($$result, $$props, $$slots) => {
  const Astro = $$result.createAstro($$Astro$1, $$props, $$slots);
  Astro.self = $$Picture;
  const defaultFormats = ["webp"], defaultFallbackFormat = "png", specialFormatsFallback = ["gif", "svg", "jpg", "jpeg"], { formats = defaultFormats, pictureAttributes = {}, fallbackFormat, ...props } = Astro.props;
  if (props.alt === void 0 || props.alt === null) throw new AstroError(ImageMissingAlt);
  const scopedStyleClass = props.class?.match(/\bastro-\w{8}\b/)?.[0];
  scopedStyleClass && (pictureAttributes.class ? pictureAttributes.class = `${pictureAttributes.class} ${scopedStyleClass}` : pictureAttributes.class = scopedStyleClass);
  const useResponsive = (props.layout ?? imageConfig.layout ?? "none") !== "none";
  useResponsive && (props.layout ??= imageConfig.layout, props.fit ??= imageConfig.objectFit ?? "cover", props.position ??= imageConfig.objectPosition ?? "center");
  for (const key in props) key.startsWith("data-astro-cid") && (pictureAttributes[key] = props[key]);
  const originalSrc = await resolveSrc(props.src), optimizedImages = await Promise.all(formats.map(async (format) => await getImage({ ...props, src: originalSrc, format, widths: props.widths, densities: props.densities })));
  let resultFallbackFormat = fallbackFormat ?? defaultFallbackFormat;
  !fallbackFormat && isESMImportedImage(originalSrc) && specialFormatsFallback.includes(originalSrc.format) && (resultFallbackFormat = originalSrc.format);
  const fallbackImage = await getImage({ ...props, format: resultFallbackFormat, widths: props.widths, densities: props.densities }), imgAdditionalAttributes = {}, sourceAdditionalAttributes = {};
  props.sizes && (sourceAdditionalAttributes.sizes = props.sizes), fallbackImage.srcSet.values.length > 0 && (imgAdditionalAttributes.srcset = fallbackImage.srcSet.attribute);
  const { class: className, ...attributes } = { ...imgAdditionalAttributes, ...fallbackImage.attributes };
  return renderTemplate`${maybeRenderHead()}<picture data-source-file="/home/rayan/bossie-ride/node_modules/astro/components/Picture.astro" data-source-line-start="114" data-source-line-end="132"${spreadAttributes(pictureAttributes)}>
	${Object.entries(optimizedImages).map(([_, image]) => {
    const srcsetAttribute = props.densities || !props.densities && !props.widths && !useResponsive ? `${image.src}${image.srcSet.values.length > 0 ? ", " + image.srcSet.attribute : ""}` : image.srcSet.attribute;
    return renderTemplate`<source${addAttribute(srcsetAttribute, "srcset")}${addAttribute(mime.lookup(image.options.format ?? image.src) ?? `image/${image.options.format}`, "type")}${spreadAttributes(sourceAdditionalAttributes)}>`;
  })}
	
	<img data-source-file="/home/rayan/bossie-ride/node_modules/astro/components/Picture.astro" data-source-line-start="131" data-source-line-end="131"${addAttribute(fallbackImage.src, "src")}${spreadAttributes(attributes)}${addAttribute(className, "class")}>
</picture>`;
}, "/home/rayan/bossie-ride/node_modules/astro/components/Picture.astro", void 0);
...
const $$Font = createComponent(($$result, $$props, $$slots) => {
  ...
  if (!componentDataByCssVariable) throw new AstroError(ExperimentalFontsNotEnabled);
  const { cssVariable, preload = false } = Astro.props, data = componentDataByCssVariable.get(cssVariable);
  if (!data) throw new AstroError({ ...FontFamilyNotFound, message: FontFamilyNotFound.message(cssVariable) });
...
}, "/home/rayan/bossie-ride/node_modules/astro/components/Font.astro", void 0);
...
const outDir = /* @__PURE__ */ new URL("file:///home/rayan/bossie-ride/dist/client/");
const serverDir = /* @__PURE__ */ new URL("file:///home/rayan/bossie-ride/dist/server/");
